...
/* Add an array of DataType definitions to the server configuration
 * (serverConfig->customDataTypes). The array is typically generated by the
 * nodeset compiler tooling, is declared const and can reside in read-only
 * memory (ROM).
 *
 * The namespaceUris argument describes the namespace indices used by the
 * NodeIds within the types array: namespaceUris[i] is the URI of the
 * namespace with index i at generation time. Index 0 is always the OPC UA
 * namespace. Unused indices may carry an empty string.
 *
 * All non-empty namespace URIs are added to the server's namespace array (if
 * not already present). If the resulting runtime namespace indices are
 * identical to the generation-time indices, the types array is linked into
 * the configuration as-is -- without copying, so that const definitions stay
 * in read-only memory. Otherwise a copy of the UA_DataType array is
 * allocated on the heap and the namespace indices are rewritten in the copy.
 * The UA_DataTypeMember arrays and the type names are never copied; they
 * contain no namespace-dependent data and remain shared with the input.
 *
 * To guarantee the zero-copy path on constrained devices, generate the types
 * array with the final namespace indices baked in (--namespaceMap option of
 * generate_datatypes.py, NAMESPACE_MAP argument of the CMake generation
 * macros) and register the namespaces in the matching order.
 *
 * Attention! When the copy-fallback is taken, pointers into the input array
 * (e.g. &UA_TYPES_DI[UA_TYPES_DI_...]) still carry the generation-time
 * NodeIds. Values built with such pointers fail the server's type checks and
 * encode with the wrong ids. In setups where the runtime namespace order is
 * not fixed, resolve the registered definition via UA_Server_findDataType.
 *
 * The method is idempotent: if the first type (with translated NodeId) is
 * already registered, UA_STATUSCODE_GOOD is returned and nothing is added. */
UA_EXPORT UA_THREADSAFE UA_StatusCode
