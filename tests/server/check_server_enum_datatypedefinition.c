...
START_TEST(dtdef_fallback_enumvalues) {
    ...
    ck_assert(UA_String_equal(&def->fields[0].name, &UA_STRING("Off")));

    /* Field 1: value=1, name="On" */
    ck_assert_int_eq(def->fields[1].value, 1);
    ck_assert(UA_String_equal(&def->fields[1].name, &UA_STRING("On")));

    /* Field 2: value=5 (gap), name="Error" */
    ck_assert_int_eq(def->fields[2].value, 5);
    ck_assert(UA_String_equal(&def->fields[2].name, &UA_STRING("Error")));

    /* displayName text must match the original EnumValueType displayName */
    ck_assert(UA_String_equal(&def->fields[0].displayName.text,
                              &UA_STRING("Off")));
    ck_assert(UA_String_equal(&def->fields[2].displayName.text,
                              &UA_STRING("Error")));
...
START_TEST(dtdef_fallback_enumstrings) {
    ...
    ck_assert(UA_String_equal(&def->fields[0].displayName.text,
                              &UA_STRING("Alpha")));
    ck_assert(UA_String_equal(&def->fields[1].displayName.text,
                              &UA_STRING("Beta")));
    ck_assert(UA_String_equal(&def->fields[2].displayName.text,
                              &UA_STRING("Gamma")));
    ...
    ck_assert(UA_String_equal(&def->fields[0].name, &UA_STRING("Alpha")));
