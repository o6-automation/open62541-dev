...
| Method | `ns=1;i=62541` under `ObjectsFolder` — HelloWorld(String) → String |
...
> **Note:** Many secure servers (including the open62541 `ci_server`) correctly
...
|---|------|----------|
| 1 | Connect anonymous (any policy) | `Good` |
| 2 | Call `ns=1;i=62541`.HelloWorld("World") on ObjectsFolder | Output contains `"Hello World"` |
| 3 | Disconnect | Clean close |
...
- **C server** trusts: `client_c`, `client_dotnet` (for both SecureChannel and session/X509 auth)
...
- **Scenario A**: C server ↔ .NET client (all tests T-1 through T-9)
- **Scenario B**: .NET server ↔ C client (all tests T-1 through T-9)

The CI workflow (`.github/workflows/interop_tests.yml`) runs both scenarios
